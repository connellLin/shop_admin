<template>
  <div>
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item
        ><a href="javascript:;">限时秒杀列表</a></el-breadcrumb-item
      >
    </el-breadcrumb>
    <div class="app-container">
      <el-button type="primary" @click="handleAdd">添加</el-button>
      <!-- 限时秒杀列表 -->
      <el-table :data="seckillData" border row-key="id" style="width: 100%">
...
export default {
  ...
  methods: {
    getData() {
      httpAuth.get("/secklist").then((res) => {
        console.log(res);
    ...
    handleEdit(id) {
      ...
      httpAuth
        ...
        .then((res) => {
          ...
          httpAuth
            .get("/goodslist", {
              fid: info.first_cateid,
              sid: info.second_cateid,
            })
            .then((res) => {
              this.goodsarr = res.list || [];
            });
    ...
    handleSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        let url = this.form.id ? "/seckedit" : "/seckadd";
        httpAuth.post(url, this.form).then((res) => {
          if (res.code === 200) {
            this.$message({
              showClose: true,
              message: res.msg,
    ...
    handleDelete(row) {
      httpAuth.post("/seckdelete", { id: row.id }).then((res) => {
        if (res.code != 200) {
          this.$message({
            showClose: true,
            message: res.msg,
